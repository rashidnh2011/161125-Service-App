...
function generateServiceReportPDF($report_id, $report_number, $pdf_path) {
    global $db, $user_data;

    try {
        // Ensure PDF path is properly formatted for TCPDF
        $clean_pdf_path = realpath($pdf_path) ?: $pdf_path;

        // Get complete report data for PDF generation (SAME QUERY AS DOWNLOAD)
        $full_query = "SELECT sr.*, c.*, u.name as technician_name, u.email as technician_email,
        ...
        $full_stmt->bindParam(":report_id", $report_id);
        $full_stmt->execute();
        $full_report = $full_stmt->fetch(PDO::FETCH_ASSOC);

        // Get service items with spares (SAME QUERY AS DOWNLOAD)
        ...
        // Generate PDF (IDENTICAL TO DOWNLOAD VERSION)
        $pdf = new CustomPDF(PDF_PAGE_ORIENTATION, PDF_UNIT, PDF_PAGE_FORMAT, true, 'UTF-8', false);

        // Set document information (IDENTICAL TO DOWNLOAD)
        $pdf->SetCreator('Service Reports System');
        $pdf->SetAuthor('ARAB SCALE TRADING L.L.C.');
        $pdf->SetTitle('Service Report #' . $report_number);

        // Set margins and breaks (IDENTICAL TO DOWNLOAD)
        ...
        // --- LOGO AND REMOTE IMAGE HANDLING (IDENTICAL TO DOWNLOAD) ---
    ...
    $customer_seal_stmt->bindParam(":customer_id", $full_report['customer_id']);
    ...
    if ($customer_seal_stmt->rowCount() > 0) {
        ...
        if (!empty($customer_seal_row['seal_image'])) {
            ...
            if (file_put_contents($customer_seal_temp_file, $seal_data) !== false) {
                ...
                if ($image_info !== false) {
                        ...
                        if ($img) {
                            ...
                            if ($width > 0 && $height > 0) {
                                ...
                                if (imagepng($transparent_img, $customer_seal_transparent_file)) {
                                    ...
                                    error_log("Successfully created transparent customer seal for customer ID: " . $full_report['customer_id']);
                                }
                    ...
                    error_log("Invalid image data in customer seal for customer ID: " . $full_report['customer_id'] . ". Error: " . ($error ? $error['message'] : 'Unknown error'));
                ...
                $error = error_get_last();
                error_log("Failed to write customer seal to temp file for customer ID: " . $full_report['customer_id'] . ". Error: " . ($error ? $error['message'] : 'Unknown error'));
            }
        }
    }

        // Process Engineer Signature for Transparency (IDENTICAL TO DOWNLOAD)
        if (!empty($full_report['engineer_signature'])) {
            try {
                $engineer_sig_data = str_replace('data:image/png;base64,', '', $full_report['engineer_signature']);
        ...
        // --- HEADER: Logo and Report Details (IDENTICAL TO DOWNLOAD) ---
        ...
        // Logo Placement (IDENTICAL TO DOWNLOAD)
        ...
        // Title Block (IDENTICAL TO DOWNLOAD)
        ...
        // Report Details Block (IDENTICAL TO DOWNLOAD)
        $right_block_width = 70;
        $label_width = 30;
        $value_width = $right_block_width - $label_width;
        $report_details_start_x = $pdf->GetPageWidth() - 15 - $right_block_width;
        $report_details_y_start = $logo_y + 2;

        // Report Number (IDENTICAL TO DOWNLOAD)
        $pdf->SetFont('helvetica', 'B', 12);
        $pdf->SetXY($report_details_start_x, $report_details_y_start);
        $pdf->Cell($right_block_width, 5, 'REPORT #' . $report_number, 0, 1, 'R');

        // Details (IDENTICAL TO DOWNLOAD)
        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetX($report_details_start_x);
        $pdf->Cell($label_width, 4, 'Date of Visit:', 0, 0, 'L');
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->Cell($value_width, 4, date('d/m/Y', strtotime($full_report['visit_date'])), 0, 1, 'R');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetX($report_details_start_x);
        $pdf->Cell($label_width, 4, 'Technician:', 0, 0, 'L');
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->Cell($value_width, 4, $full_report['technician_name'] ?: 'N/A', 0, 1, 'R');

        $pdf->SetFont('helvetica', '', 9);
        $pdf->SetX($report_details_start_x);
        $pdf->Cell($label_width, 4, 'Type:', 0, 0, 'L');
        $pdf->SetFont('helvetica', 'B', 9);
        $pdf->Cell($value_width, 4, ucfirst($full_report['type']), 0, 1, 'R');

        // Service Time (IDENTICAL TO DOWNLOAD)
        if ($full_report['duration_seconds']) {
            $pdf->SetFont('helvetica', '', 9);
            $pdf->SetX($report_details_start_x);
            $pdf->Cell($label_width, 4, 'Service Time:', 0, 0, 'L');
            $pdf->SetFont('helvetica', 'B', 9);
            $hours = floor($full_report['duration_seconds'] / 3600);
            $minutes = floor(($full_report['duration_seconds'] % 3600) / 60);
            $service_time = sprintf('%dh %dm', $hours, $minutes);
            $pdf->Cell($value_width, 4, $service_time, 0, 1, 'R');
        }

        // Set Y position (IDENTICAL TO DOWNLOAD)
        $final_header_y = max($logo_y + $logo_height, $pdf->GetY());
        $pdf->SetY($final_header_y);
        $pdf->Ln(2);

        // Customer Details Section (IDENTICAL TO DOWNLOAD)
        $pdf->SetFont('helvetica', 'B', 14);
        ...
        $pdf->Cell(0, 6, $full_report['name'], 0, 1);
        $pdf->SetFont('helvetica', '', 10);

        $pdf->Cell(40, 6, 'Contact Person:', 0, 0);
        $pdf->Cell(0, 6, $full_report['contact_person'], 0, 1);

        if ($full_report['phone']) {
            $pdf->Cell(40, 6, 'Phone:', 0, 0);
            $pdf->Cell(0, 6, $full_report['phone'], 0, 1);
        }

        if ($full_report['email']) {
            $pdf->Cell(40, 6, 'Email:', 0, 0);
            $pdf->Cell(0, 6, $full_report['email'], 0, 1);
        }

        if ($full_report['address']) {
            $pdf->Cell(40, 6, 'Address:', 0, 0);
            $address = $full_report['address'] . ', ' . $full_report['city'] . ', ' . $full_report['state'];
            if ($full_report['pincode']) {
                $address .= ' - ' . $full_report['pincode'];
            }
            $pdf->MultiCell(0, 6, $address, 0, 'L');
        }

        if ($full_report['start_address']) {
            $pdf->Cell(40, 6, 'Service Location:', 0, 0);
            $pdf->MultiCell(0, 6, $full_report['start_address'], 0, 'L');
        }

        $pdf->Ln(5);

        // Service Summary Section (IDENTICAL TO DOWNLOAD)
        if ($full_report['start_time'] || $full_report['payment_amount'] || $full_report['invoice_number']) {
            $pdf->SetFont('helvetica', 'B', 14);
            $pdf->SetFillColor(200, 220, 255);
            ...
            if ($full_report['start_time'] && $full_report['end_time']) {
                $pdf->Cell(40, 6, 'Service Started:', 0, 0);
                $pdf->Cell(0, 6, date('d/m/Y H:i', strtotime($full_report['start_time'])), 0, 1);

                $pdf->Cell(40, 6, 'Service Completed:', 0, 0);
                $pdf->Cell(0, 6, date('d/m/Y H:i', strtotime($full_report['end_time'])), 0, 1);
            ...
            if ($full_report['invoice_number'] || $full_report['payment_amount']) {
                $pdf->Ln(2);
                if ($full_report['invoice_number']) {
                    $pdf->Cell(40, 6, 'Invoice Number:', 0, 0);
                    $pdf->SetFont('helvetica', 'B', 10);
                    $pdf->Cell(0, 6, $full_report['invoice_number'], 0, 1);
                    $pdf->SetFont('helvetica', '', 10);
                }

                if ($full_report['payment_amount']) {
                    $pdf->Cell(40, 6, 'Service Amount:', 0, 0);
                    $pdf->SetFont('helvetica', 'B', 10);
                    $pdf->Cell(50, 6, 'AED ' . number_format($full_report['payment_amount'], 2), 0, 0);

                    $pdf->SetFont('helvetica', '', 9);
                    $status_color = $full_report['payment_status'] === 'paid' ? array(0, 128, 0) : array(255, 0, 0);
                    $pdf->SetTextColor($status_color[0], $status_color[1], $status_color[2]);
                    $pdf->Cell(0, 6, '(' . ucfirst($full_report['payment_status']) . ')', 0, 1);
                    $pdf->SetTextColor(0, 0, 0);
                }
            }

            if ($full_report['location_verified'] !== null) {
                $pdf->Cell(40, 6, 'Location Verified:', 0, 0);
                $pdf->SetFont('helvetica', 'B', 10);
                $verification_text = $full_report['location_verified'] ? 'Yes' : 'Pending';
        ...
        // Service Items Section (IDENTICAL TO DOWNLOAD)
        $pdf->SetFont('helvetica', 'B', 14);
        $pdf->SetFillColor(200, 220, 255);
        ...
        foreach ($items as $item) {
            ...
            // Spares used Sub-Section (IDENTICAL TO DOWNLOAD)
        ...
        // Cost Summary Section (IDENTICAL TO DOWNLOAD)
        if ($total_spares_cost > 0 || $full_report['payment_amount']) {
            $pdf->SetFont('helvetica', 'B', 14);
            $pdf->SetFillColor(200, 220, 255);
            ...
            if ($full_report['payment_amount']) {
                ...
                $pdf->SetFont('helvetica', 'B', 12);
                $pdf->Cell(0, 6, 'AED ' . number_format($full_report['payment_amount'], 2), 0, 1);
                $pdf->SetFont('helvetica', '', 10);
            }
        ...
        // Notes Section (IDENTICAL TO DOWNLOAD)
        if ($full_report['notes']) {
            $pdf->SetFont('helvetica', 'B', 14);
            $pdf->SetFillColor(200, 220, 255);
            $pdf->Cell(0, 8, 'General Notes', 0, 1, 'L', 1);
            $pdf->SetFont('helvetica', 'I', 10);
            $pdf->MultiCell(0, 6, $full_report['notes'], 0, 'L');
            $pdf->Ln(5);
        }

        // Signatures Section (IDENTICAL TO DOWNLOAD)
        if ($full_report['engineer_signature'] || $full_report['customer_signature'] || $seal_path_to_use || $customer_seal_path_to_use) {
            $pdf->SetFont('helvetica', 'B', 14);
            $pdf->SetFillColor(200, 220, 255);
...
Customer Seal]', 1, 'C', false, 1, $x_customer_seal, $y_start_for_images);
            ...
            // Display Engineer Signature Image
            ...
            } elseif (!empty($full_report['engineer_signature'])) {
                try {
            ...
            if (!empty($full_report['customer_signature'])) {
                try {
                    $customer_sig_file = $temp_dir . '/customer_sig_' . $report_id . '.png';
                    $customer_sig_data = str_replace('data:image/png;base64,', '', $full_report['customer_signature']);
            ...
            $y_start_for_text = $y_start_for_images + $max_seal_height + 2;
            $current_y = $y_start_for_text;

            // Company Seal Line & Name
            if ($seal_path_to_use) {
                $pdf->SetFont('helvetica', 'B', 10);
                $pdf->SetXY($x_seal, $current_y);
                // Draw a line
                ...
                $pdf->SetFont('helvetica', 'B', 10);
                $pdf->SetXY($x_seal, $current_y);
                $pdf->Cell($seal_width, 5, 'Company Seal', 0, 0, 'C');
            }

            // Customer Seal Line & Name
            ...
            // Engineer Signature Line & Name
            if ($full_report['engineer_signature'] || $seal_path_to_use) {
                $pdf->SetFont('helvetica', 'B', 10);
                ...
                $pdf->Cell($seal_width, 4, $full_report['technician_name'] ?: 'N/A', 0, 0, 'C');
            }

            // Customer Signature Line & Name
            if ($full_report['customer_signature']) {
                $pdf->SetFont('helvetica', 'B', 10);
                ...
                $signature_name = isset($full_report['signature_person_name']) && !empty($full_report['signature_person_name']) ? $full_report['signature_person_name'] : $full_report['contact_person'];
                $signature_contact = isset($full_report['signature_person_contact']) && !empty($full_report['signature_person_contact']) ? $full_report['signature_person_contact'] : '';
        ...
        // Clean up temporary files (IDENTICAL TO DOWNLOAD)
        ...
        // Generate PDF content as string (IDENTICAL TO DOWNLOAD)
        $pdf_content = $pdf->Output('', 'S');

        // Save PDF content to file (IDENTICAL TO DOWNLOAD)
        $bytes_written = file_put_contents($clean_pdf_path, $pdf_content);

        if ($bytes_written === false || $bytes_written == 0) {
            throw new Exception('Failed to write PDF to file');
        }

        // Verify file was created and has content (IDENTICAL TO DOWNLOAD)
        if (!file_exists($clean_pdf_path) || filesize($clean_pdf_path) == 0) {
...
if (!isset($data->report_id) || !isset($data->emails) || empty($data->emails)) {
    http_response_code(400);
    echo json_encode(array("success" => false, "error" => "Report ID and email addresses required"));
    exit();
}

try {
    // Verify report exists and user has access
    $check_query = "SELECT sr.id, sr.report_number, sr.status, c.name as customer_name 
                    FROM service_reports sr
                    LEFT JOIN customers c ON sr.customer_id = c.id
                    WHERE sr.id = :report_id";
    if ($user_data['role'] !== 'admin') {
        $check_query .= " AND technician_id = :technician_id";
    }
    ...
    // Create uploads directory if it doesn't exist (use absolute path from current file)
    $uploads_dir = dirname(__FILE__) . '/../uploads/reports/';
    if (!file_exists($uploads_dir)) {
        mkdir($uploads_dir, 0777, true);
    }

    // Generate PDF file using the same logic as download endpoint
    $pdf_path = $uploads_dir . 'Service_Report_' . $report['report_number'] . '.pdf';

    try {
        // Generate PDF using the same endpoint as download
        $pdf_response = generateServiceReportPDF($data->report_id, $report['report_number'], $pdf_path);

        if (!$pdf_response['success']) {
            throw new Exception('PDF generation failed: ' . $pdf_response['error']);
        }

        // PDF is already saved to file, use the path directly
    ...
    $email_result = $email_service->sendServiceReport(
        $data->emails,
    ...
    if (!$email_result['success']) {
        ...
        echo json_encode(array("success" => false, "error" => $email_result['error']));
        exit();
    }
    
    // Log email send
    $log_query = "INSERT INTO email_logs (report_id, sender_id, recipients, sent_at, status) 
                  VALUES (:report_id, :sender_id, :recipients, NOW(), :status)";
    $log_stmt = $db->prepare($log_query);
    $log_stmt->bindParam(":report_id", $data->report_id);
    $log_stmt->bindParam(":sender_id", $user_data['id']);
    $recipients_json = json_encode($data->emails);
    $log_stmt->bindParam(":recipients", $recipients_json);
    $email_status = $email_result['success'] ? 'sent' : 'failed';
    $log_stmt->bindParam(":status", $email_status);
    $log_stmt->execute();
    
    // Update report status to sent if not already
    if ($report['status'] !== 'sent') {
        $update_query = "UPDATE service_reports SET status = 'sent' WHERE id = :report_id";
    ...
    $audit_query = "INSERT INTO audit_logs (user_id, action, target_table, target_id, details) 
                    VALUES (:user_id, 'send_email', 'service_reports', :report_id, :details)";
    $audit_stmt = $db->prepare($audit_query);
    $audit_stmt->bindParam(":user_id", $user_data['id']);
    $audit_stmt->bindParam(":report_id", $data->report_id);
    $audit_details = json_encode(array("recipients" => $data->emails, "message" => $data->message ?? ''));
    $audit_stmt->bindParam(":details", $audit_details);
    $audit_stmt->execute();
    
    echo json_encode(array(
        "success" => true,
        "message" => "Report sent successfully to " . count($data->emails) . " recipient(s)"
    ));
    
} catch (Exception $e) {
    http_response_code(500);
    echo json_encode(array("success" => false, "error" => "Failed to send report: " . $e->getMessage()));
}
?>